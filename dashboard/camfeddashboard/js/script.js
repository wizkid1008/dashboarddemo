...
// KPI 1.3 — Total Girls / Boys: from local demo KPI rows
...
function renderEducationReachCharts() {
  ...
  // Total Girls / Boys: from local demo KPI rows (KPI 1.3)
...
function renderLearnerGuideProgrammeCharts() {
  ...
  setTimeout(() => {
    ...
    erBar('lg-training-chart', a,
      [{ label: 'SHF Agriculture Trained', data: lgVals, backgroundColor: '#D9A441' }],
      { legend: true });
...
function renderLivelihoodsReachCharts() {
  ...
  setTimeout(() => {
    ...
    erBar('l2lr-loans-chart', a, [
      { label: 'SHF Agriculture Kiva Loans', data: kivaVals, backgroundColor: '#8FCB62' },
      { label: 'SHF Agriculture RIF Loans',  data: rifVals,  backgroundColor: '#0D4F6C' }
    ], { legend: true });
  }, 0);
...
function buildL1() {
  ...
  if (single) {
    ...
    bar('l1-lg-chart', a, [
      {label:'SHF Agriculture', data:a.map(n=>{ const t=ddQC(LG,n), d=(D.kpi19.camfed[n]||0)+(D.kpi19.govt[n]||0)||1; return ddSplit(t,D.kpi19.camfed[n]||0,d); }), backgroundColor:'#0D4F6C'},
